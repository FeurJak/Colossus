...
use crate::{
    ...
    core::{
        Encapsulations, KmacSignature, MIN_TRACING_LEVEL, MasterPublicKey, MasterSecretKey,
        RightPublicKey, RightSecretKey, SHARED_SECRET_LENGTH, SIGNATURE_LENGTH, SIGNING_KEY_LENGTH,
        TAG_LENGTH, TracingSecretKey, UserId, UserSecretKey, XEnc,
        kem::{self, MlKem},
    },
...
fn h_decaps(
    ...
    tag: &[u8; TAG_LENGTH],
    encs: &[(<kem::MlKem as Kem>::Encapsulation, [u8; SHARED_SECRET_LENGTH])],
...
pub fn rekey(
    ...
    for r in rights {
        if msk.secrets.contains_key(&r) {
            let is_hybridized =
                msk.secrets.get_latest(&r).map(|(_, k)| k.is_hybridized()).ok_or_else(|| {
                    Error::OperationNotPermitted(format!("no current key for coordinate {r:#?}"))
                })?;

            msk.secrets.insert(r, (true, RightSecretKey::random(rng, is_hybridized)?));
        } else {
            return Err(Error::OperationNotPermitted(
