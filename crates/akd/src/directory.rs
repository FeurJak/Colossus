    ...
    V: VRFKeyStorage,
{
    ...
    /// Retrieves the [Azks]
    #[tracing::instrument(skip_all)]
    pub(crate) async fn retrieve_azks(&self) -> Result<Azks, crate::AkdError> {
        Directory::<TC, S, V>::get_azks_from_storage(&self.storage, false).await
    }
    ...
    async fn get_azks_from_storage(
        storage: &StorageManager<S>,
        ignore_cache: bool,
    ) -> Result<Azks, crate::AkdError> {
        let got = if ignore_cache {
            storage.get_direct::<Azks>(&crate::append_only_zks::DEFAULT_AZKS_KEY).await?
        } else {
            storage.get::<Azks>(&crate::append_only_zks::DEFAULT_AZKS_KEY).await?
        };
        match got {
