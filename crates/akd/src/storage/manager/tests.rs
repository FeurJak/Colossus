...
use crate::storage::{traits::StorageUtil, types::*};
...
async fn test_storage_manager_transaction() {
    ...
    storage_manager.batch_set(records).await.expect("Failed to set batch of records");
    ...
    storage_manager.commit_transaction().await.expect("Failed to commit transaction");
