...
use super::{Attribute, AttributeStatus, Dict, Dimension, EncryptionHint, Error, Name};
...
mod serialization {
    ...
    impl Serializable for Attribute {
        ...
        fn read(de: &mut Deserializer) -> Result<Self, Self::Error> {
            ...
            let encryption_hint = if 0 == hint {
                ...
                return Err(Error::ConversionFailed(format!("erroneous hint value {hint}")));
            };
            ...
            let write_status = if 0 == status {
                ...
                return Err(Error::ConversionFailed(format!("erroneous status value {hint}")));
            };

            Ok(Self { id, encryption_hint, write_status })
        }
    }
    ...
    fn test_dimension_serialization() {
        ...
        d.add_attribute("A".to_string(), EncryptionHint::Classic, None, 0).unwrap();
        ...
        d.add_attribute("A".to_string(), EncryptionHint::Classic, None, 0).unwrap();
        d.add_attribute("B".to_string(), EncryptionHint::Hybridized, None, 1).unwrap();
        d.add_attribute("C".to_string(), EncryptionHint::Hybridized, None, 2).unwrap();
        test_serialization(&d).unwrap();
    }
}
